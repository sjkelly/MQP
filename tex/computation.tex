...
system" can be either dynamic or static, which indicates if the programmer
needs to specify to the program compiler how data is transformed in a function.
Generic functions allow a single function name, for example "sum'', to have
multiple defintions with execution contigent upon the matching of argument
types. The definition of a parametric type is more nuanced, but generally
...
provers.\cite{Harper_1997}. A basic example of inference in Julia is shown below:
\begin{lstlisting}
